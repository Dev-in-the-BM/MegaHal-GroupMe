...
export class MegaHAL {
	...
	public async train(linesOrFilename: string[] | string): Promise<void> {
		if (typeof linesOrFilename === 'string') {
			throw new Error('fs.readFile not supported in serverless environment');
	...
	public reply(input: string, error = '...'): string {
		...
		const kwSymbols = Array.from(Keywords.extract(norms))
			.map((kw) => this.dictionary[kw!])
			.filter(notNull);
	 ...
	 */
	private _randomWalk(model: SoothPredictor, staticContext: number[], kwSymbols: number[]): number[] {
	 ...
	 */
	private _decompose(line: string | undefined | null, maxLen = 1024): [string[] | null, string[] | null, string[] | null] {
	...
	private _rewrite(normSymbols: number[]): string | null {
		const decode: Record<number, string> = Object.fromEntries(Object.entries(this.dictionary).map((x) => x.reverse()));
